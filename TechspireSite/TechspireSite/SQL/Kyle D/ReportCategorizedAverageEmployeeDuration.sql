...
SELECT  'All' AS 'Employee Classifcation',
        ...
        SUM(DATEDIFF(day, begin_date, IIF(end_date IS NOT NULL , end_date, getdate() ))) / COUNT(Employee.id) AS 'Avg Employment in Days',
		SUM(DATEDIFF(month, begin_date, IIF(end_date IS NOT NULL , end_date, getdate() ))) / COUNT(Employee.id) AS 'Avg Employment in Months'
...
SELECT  'Store 1: Hot Breads' AS 'Employee Classifcation',
		SUM(DATEDIFF(day, Employee.begin_date, IIF(Employee.end_date IS NOT NULL , Employee.end_date, getdate() ))) AS 'Total Employee Days',
		COUNT(Employee.id) AS 'Total Employee Count',
        SUM(DATEDIFF(day, Employee.begin_date, IIF(Employee.end_date IS NOT NULL , Employee.end_date, getdate() ))) / COUNT(Employee.id) AS 'Avg Employment in Days',
		SUM(DATEDIFF(month, Employee.begin_date, IIF(Employee.end_date IS NOT NULL , Employee.end_date, getdate() ))) / COUNT(Employee.id) AS 'Avg Employment in Months'
...
SELECT  'Store 2: Red Berries' AS 'Employee Classifcation',
		SUM(DATEDIFF(day, Employee.begin_date, IIF(Employee.end_date IS NOT NULL , Employee.end_date, getdate() ))) AS 'Total Employee Days',
		COUNT(Employee.id) AS 'Total Employee Count',
        SUM(DATEDIFF(day, Employee.begin_date, IIF(Employee.end_date IS NOT NULL , Employee.end_date, getdate() ))) / COUNT(Employee.id) AS 'Avg Employment in Days',
		SUM(DATEDIFF(month, Employee.begin_date, IIF(Employee.end_date IS NOT NULL , Employee.end_date, getdate() ))) / COUNT(Employee.id) AS 'Avg Employment in Months'
