...
--Row Number, Order Date, Product Name, Quantity, Product Price, Order Total, First Name, Last Name, Customer Phone Number, Store Name 
--,,,right,right,right,,,phone, 

SELECT ROW_NUMBER() OVER(ORDER BY ("Order".order_date) desc) AS Num_Row,
"Order".order_date AS Order_Date, Product.product_name AS Product_Name, Orderline.quantity AS Quantity, (concat('$', cast(STR(Product.product_price,20,2) AS decimal(18,2)))) AS Product_Price, (concat('$', cast(STR("Order".final_total,20,2) AS decimal(18,2)))) AS Order_Total, Customer.first_name AS First_Name, Customer.last_name AS Last_Name, Customer.phone_number AS Customer_Phone_Number,
